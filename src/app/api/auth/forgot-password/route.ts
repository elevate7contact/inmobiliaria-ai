...
export async function POST(request: Request) {
  try {
    ...
    // 2. Enviar email via Brevo REST API
    const brevoRes = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        sender: {
          name: "Chatinmuebles",
          email: process.env.SMTP_USER, // vistagpt1@hotmail.com (verificado en Brevo)
        },
        to: [{ email }],
    ...
    if (!brevoRes.ok) {
      ...
      console.error("Brevo error:", JSON.stringify(brevoError));
